...
public class MigrationSquasher
{
    ...
    public async Task SquashMigrationsAsync(string migrationName)
    {
        try
        {
            ...
            await CreateConsolidatedMigrationAsync(migrationName, extractedSchema, extractedDown);

            // Step 8: Generate database update script
            GenerateDatabaseUpdateScript(migrationName);
    ...
    private async Task CreateConsolidatedMigrationAsync(string migrationName, string extractedSchema, string extractedDown)
    {
...
{extractedDown}
    ...
    private string GenerateDesignerContent(string className, string migrationName)
    {
        ...
        if (!string.IsNullOrWhiteSpace(modelSnapshot))
        {
            try
            {
                var buildModelMatch = Regex.Match(modelSnapshot, @"protected\s+override\s+void\s+BuildModel\s*\(\s*ModelBuilder\s+modelBuilder\s*\)\s*{(.*?)(?=^\s*}\s*$)", RegexOptions.Multiline | RegexOptions.Singleline);
                if (buildModelMatch.Success)
                {
                    var buildModelContent = buildModelMatch.Groups[1].Value;
                    var version = "9.0.11"; // Default EF Core version
...
{{
    ...
    {{
        ...
        {{
            ...
            modelBuilder
                .HasAnnotation(""ProductVersion"", ""{version_default}"")
    ...
    private void GenerateDatabaseUpdateScript(string migrationName)
    {
        var timestamp = DateTime.UtcNow.ToString("yyyyMMddHHmmss");
...
VALUES ('{migrationId}', '9.0.0'); -- Update ProductVersion to match your EF Core version
